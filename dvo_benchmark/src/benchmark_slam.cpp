...
dvo::core::RgbdImagePyramidPtr
load (dvo::core::RgbdCameraPyramid& camera, std::string rgb_file, std::string depth_file)
{
    ...
    if (rgb.total () == 0 || depth.total () == 0)
        return dvo::core::RgbdImagePyramidPtr ();
...
public:
    ...
    void renderWhileSwitchAndNotTerminated (dvo::visualization::CameraTrajectoryVisualizerInterface* visualizer,
...
: nh_ (nh), nh_vis_ (nh, "dvo_vis"), nh_private_ (nh_private),
  trajectory_out_ (0), groundtruth_reader_ (0), rgbdpair_reader_ (0),
  dump_camera_pose_ (false), load_camera_pose_ (false)
{
}
...
bool BenchmarkNode::configure ()
{
    ...
    {
        if (nh_private_.getParam ("trajectory_file", cfg_.TrajectoryFile) &&
            !cfg_.TrajectoryFile.empty ())
        {
    ...
    if (cfg_.RenderVideo)
    {
        if (!nh_private_.getParam ("video_folder", cfg_.VideoFolder) ||
            cfg_.VideoFolder.empty ())
        {
    ...
    if (cfg_.ShowGroundtruth)
    {
        if (nh_private_.getParam ("groundtruth_file", cfg_.GroundtruthFile))
        {
            ...
            {
                std::cerr << "Failed to open '" << cfg_.GroundtruthFile << "'!"
                          << std::endl;
                return false;
            }
        }
...
void BenchmarkNode::processInput (dvo::visualization::CameraTrajectoryVisualizerInterface* visualizer)
{
    if (cfg_.VisualizationRequired ())
    {
        if (dump_camera_pose_.value ())
        {
            ...
            {
                std::cerr << "saving camera pose to '" << cfg_.CameraFile << "'"
                          << std::endl;

                std::vector<pcl::visualization::Camera> cams;
                ((dvo::visualization::PclCameraTrajectoryVisualizer*)visualizer)->visualizer ().getCameras (
                cams);

                // same output format as PCLVisualizerInteractorStyle (when
                // pressing 'c' key)
                std::ofstream camera_pose_file (cfg_.CameraFile.c_str ());
                camera_pose_file
                << cams[0].clip[0] << "," << cams[0].clip[1] << "/"
                << cams[0].focal[0] << "," << cams[0].focal[1] << ","
                << cams[0].focal[2] << "/" << cams[0].pos[0] << "," << cams[0].pos[1]
                << "," << cams[0].pos[2] << "/" << cams[0].view[0] << ","
                << cams[0].view[1] << "," << cams[0].view[2] << "/" << cams[0].fovy
            ...
            {
                std::cerr
                << "Can't save camera pose, set the 'camera_file' parameter!"
                << std::endl;
            }
        ...
        if (load_camera_pose_.value ())
        {
            if (!cfg_.CameraFile.empty ())
            {
                std::cerr << "loading camera pose from '" << cfg_.CameraFile
                          << "'" << std::endl;
            ...
            {
                std::cerr
                << "Can't load camera pose, set the 'camera_file' parameter!"
                << std::endl;
            }
...
void BenchmarkNode::createReferenceCamera (dvo::visualization::CameraTrajectoryVisualizerInterface* visualizer,
                                           const dvo::core::RgbdImage& img,
                                           const Eigen::Affine3d& pose)
{
...
void BenchmarkNode::run ()
{
    ...
    if (cfg_.VisualizationRequired ())
    {
        ...
        graph_visualizer = new dvo_slam::visualization::GraphVisualizer (
        *dynamic_cast<dvo_ros::visualization::RosCameraTrajectoryVisualizer*>(visualizer));
        //((dvo::visualization::PclCameraTrajectoryVisualizer*)
        //visualizer)->bindSwitchToKey(pause_switch, "p");
        //((dvo::visualization::PclCameraTrajectoryVisualizer*)
        //visualizer)->bindSwitchToKey(dump_camera_pose_, "s");
        //
        // if(cfg_.RenderVideo)
        //{
        //  ((dvo::visualization::PclCameraTrajectoryVisualizer*)
        //  visualizer)->bindSwitchToKey(load_camera_pose_, "l");
        //  ((dvo::visualization::PclCameraTrajectoryVisualizer*)
        //  visualizer)->visualizer().getRenderWindow()->SetSize(1280, 960);
        //}
    }
    else
    {
    ...
    dvo::util::IdGenerator frame_ids (cfg_.VideoFolder +
                                      std::string ("/frame_"));

    // setup camera parameters
    // TODO: load from file
    // dvo::core::IntrinsicMatrix intrinsics =
    // dvo::core::IntrinsicMatrix::create(525.0f, 525.0f, 320.0f, 240.0f);
    // fr1
    dvo::core::IntrinsicMatrix intrinsics =
    ...
    // fr2
    // dvo::core::IntrinsicMatrix intrinsics =
    // dvo::core::IntrinsicMatrix::create(520.9f, 521.0f, 325.1f, 249.7f);

    // fr3
    // dvo::core::IntrinsicMatrix intrinsics =
    // dvo::core::IntrinsicMatrix::create(535.4f, 539.2f, 320.1f, 247.6f);
    ...
    dvo_slam::KeyframeSlamConfig dynreconfg_slam_cfg =
    dvo_slam::KeyframeSlamConfig::__getDefault__ ();
    ...
    ROS_WARN_STREAM_NAMED ("config",
                           "tracker config: \""
                           << keyframe_tracker.trackingConfiguration () << "\"");
    ROS_WARN_STREAM_NAMED ("config",
                           "frontend config: \""
                           << keyframe_tracker.keyframeSelectionConfiguration () << "\"");
    ROS_WARN_STREAM_NAMED ("config",
                           "backend config: \""
                           << keyframe_tracker.mappingConfiguration () << "\"");
    ...
    std::string optimized_trajectory_file =
    cfg_.RgbdPairFile.substr (0, cfg_.RgbdPairFile.find_last_of (".")) +
    "_opt_traj_final.txt";
    std::string edge_error_file =
    cfg_.RgbdPairFile.substr (0, cfg_.RgbdPairFile.find_last_of (".")) +
    "_error.txt";
    ...
    std::string folder =
    cfg_.RgbdPairFile.substr (0, cfg_.RgbdPairFile.find_last_of ("/") + 1);
         ...
         ++it)
    {
        ...
        if (cfg_.ShowGroundtruth)
        {
            ...
            .show (cfg_.ShowEstimate ?
                   dvo::visualization::CameraVisualizer::ShowCamera :
        ...
        //  visualizer)->render(5);
        //  ((dvo::visualization::PclCameraTrajectoryVisualizer*)
        //  visualizer)->visualizer().saveScreenshot(frame_ids.next() +
        //  std::string(".png"));
        //}
    }
    sw_online.stop ();
    ...
    dvo_slam::serialization::FileSerializer<dvo_slam::serialization::EdgeErrorSerializer> error_serializer (
    edge_error_file);
