...
CameraDenseTracker::CameraDenseTracker(ros::NodeHandle& nh,
    ...
    :  //  CameraBase(nh, nh_private),
      ...
      use_dense_tracking_estimate_(true),
      connected(false),
      nh_(nh) {
  ...
  //  ROS_INFO_STREAM("transformation: base_link -> asus" << std::endl <<
  //  from_baselink_to_asus.matrix());

  //  pose_sub_ =
  //  nh.subscribe<geometry_msgs::PoseWithCovarianceStamped>("pelican/pose", 1,
  //  &CameraDenseTracker::handlePose, this);
  ...
  //  vis_ =
  //  std::make_shared<dvo_ros::visualization::RosCameraTrajectoryVisualizer>
  //  (nh_);
  vis_ = new dvo_ros::visualization::RosCameraTrajectoryVisualizer(nh_);
}

CameraDenseTracker::~CameraDenseTracker() { delete vis_; }

bool CameraDenseTracker::hasChanged(
...
void CameraDenseTracker::reset(
    const sensor_msgs::CameraInfo::ConstPtr& camera_info_msg) {
  // IntrinsicMatrix intrinsics = IntrinsicMatrix::create(camera_info_msg->K[0],
...
void CameraDenseTracker::handleConfig(dvo_ros::CameraDenseTrackerConfig& config,
                                      uint32_t level) {
  if (level == 0) return;
...
void CameraDenseTracker::handlePose(
...
void CameraDenseTracker::handleImages(
  ...
  if (success) {
    ...
    ROS_INFO_STREAM("estimate success");
  ...
  publishTransform(h,
                   accumulated_transform /**from_baselink_to_asus.inverse()*/,
                   "base_link_estimate");
  //  publishTransform(rgb_image_msg->header, first_transform.inverse() *
  //  accumulated_transform, "asus_estimate");
