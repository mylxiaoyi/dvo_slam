...
YamlArrayFmt (Eigen::FullPrecision, Eigen::DontAlignCols, ",", ",", "", "", "[", "]");

DenseTracker::DenseTracker (const Config& config)
: itctx_ (cfg), weight_calculation_ (), selection_predicate_ (),
...
void DenseTracker::configure (const Config& config)
{
    ...
    if (cfg.UseWeighting)
    {
        weight_calculation_.scaleEstimator (ScaleEstimators::get (cfg.ScaleEstimatorType))
        .scaleEstimator ()
        ->configure (cfg.ScaleEstimatorParam);

        weight_calculation_.influenceFunction (InfluenceFunctions::get (cfg.InfluenceFuntionType))
        .influenceFunction ()
        ->configure (cfg.InfluenceFunctionParam);
    }
    else
    {
        weight_calculation_.scaleEstimator (ScaleEstimators::get (ScaleEstimators::Unit))
        .influenceFunction (InfluenceFunctions::get (InfluenceFunctions::Unit));
    }
}

bool DenseTracker::match (RgbdImagePyramid& reference,
                          ...
                          dvo::DenseTracker::Result& result)
{
    ...
    name << std::setiosflags(std::ios::fixed) << std::setprecision(2) <<
    current.timestamp() << "_error.avi";

    cv::Size s =
    reference.getRgbdImagePyramid().level(size_t(cfg.LastLevel)).intensity.size();
    cv::Mat video_frame(s.height, s.width * 2, CV_32FC1), video_frame_u8;
    cv::VideoWriter vw(name.str(), CV_FOURCC('P','I','M','1'), 30,
    video_frame.size(), false);
    float rgb_max = 0.0;
    float depth_max = 0.0;

    std::stringstream name1;
    name1 << std::setiosflags(std::ios::fixed) << std::setprecision(2) <<
    current.timestamp() << "_ref.png";

    cv::imwrite(name1.str(), current.level(0).rgb);

    std::stringstream name2;
    name2 << std::setiosflags(std::ios::fixed) << std::setprecision(2) <<
    current.timestamp() << "_cur.png";
    ...
    */
    ...
    for (itctx_.Level = cfg.FirstLevel; itctx_.Level >= cfg.LastLevel; --itctx_.Level)
    {
        ...
        // reset error after every pyramid level? yes because errors from
        // different levels are not comparable
        itctx_.Iteration = 0;
        ...
        wcur << 1.0f / 255.0f, 1.0f, wcur_id* K.fx () / 255.0f,
        wcur_id * K.fy () / 255.0f, wcur_zd * K.fx (), wcur_zd * K.fy (), 0.0f, 0.0f;
        wref << -1.0f / 255.0f, -1.0f, wref_id* K.fx () / 255.0f,
        wref_id * K.fy () / 255.0f, wref_zd * K.fx (), wref_zd * K.fy (), 0.0f, 0.0f;
        ...
        do
        {
            ...
            if (debug)
            {
                ...
                first_point, last_point, cur, K, transformf, wref, wcur, compute_residuals_result);
            }
            else
            {
                dvo::core::computeResidualsSse (
                first_point, last_point, cur, K, transformf, wref, wcur, compute_residuals_result);
            }
            ...
            float ll =
            computeCompleteDataLogLikelihood (compute_residuals_result.first_residual,
                                              compute_residuals_result.last_residual,
                                              weights.begin (),
                                              mean,
                                              precision);
            ...
            total_error = -ll; // iteration_stats.TDistributionLogLikelihood +
            ...
            for (PointIterator e_it = compute_residuals_result.first_point_error;
                 e_it != compute_residuals_result.last_point_error;
                 ++e_it, ++w_it)
            {
    ...
    last_level.TerminationCriterion != TerminationCriteria::LogLikelihoodDecreased ?
    ...
    last_iteration.TDistributionLogLikelihood + last_iteration.PriorLogLikelihood;

    return success;
}

cv::Mat DenseTracker::computeIntensityErrorImage (dvo::core::RgbdImagePyramid& reference,
                                                  dvo::core::RgbdImagePyramid& current,
                                                  const dvo::core::AffineTransformd& transformation,
                                                  size_t level)
{
    ...
    dvo::core::computeResidualsAndValidFlagsSse (
    first_point, last_point, cur, K, transformation.cast<float>(), wref, wcur, compute_residuals_result);

    cv::Mat result = cv::Mat::zeros (reference.level (level).intensity.size (), CV_32FC1),
            debug_idx;
...
inline void
DenseTracker::computeJacobianOfProjectionAndTransformation (const Vector4& p, Matrix2x6& j)
{
    NumType z = 1.0f / p (2);
    ...
    j (0, 4) =
    1.0f - j (0, 2) * p (0); // j(0, 4) =  (1.0 + p(0) * p(0) * z_sqr);
    j (0, 5) = -p (1) * z;
    ...
    j (1, 2) = -p (1) * z_sqr;
    j (1, 3) =
    -1.0f + j (1, 2) * p (1); // j(1, 3) = -(1.0 + p(1) * p(1) * z_sqr);
    j (1, 4) = -j (0, 3); // j(1, 4) =  p(0) * p(1) * z_sqr;
    j (1, 5) = p (0) * z;
}

inline void
DenseTracker::compute3rdRowOfJacobianOfTransformation (const Vector4& p, Vector6& j)
{
    j (0) = 0.0;
    j (1) = 0.0;
    j (2) = 1.0;
