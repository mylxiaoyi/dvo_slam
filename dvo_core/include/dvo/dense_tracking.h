...
public:
    ...
    cv::Mat computeIntensityErrorImage (dvo::core::RgbdImagePyramid& reference,
                                        dvo::core::RgbdImagePyramid& current,
...
std::ostream& operator<<(std::basic_ostream<CharT, Traits>& out,
                         const dvo::DenseTracker::Config& config)
{
    out << "First Level = " << config.FirstLevel << ", Last Level = " << config.LastLevel
...
template <typename CharT, typename Traits>
std::ostream& operator<<(std::basic_ostream<CharT, Traits>& o,
                         ...
                         const dvo::DenseTracker::LevelStats& s)
{
    ...
    for (dvo::DenseTracker::IterationStatsVector::const_iterator it = s.Iterations.begin ();
         it != s.Iterations.end ();
         ++it)
    {
        o << *it;
...
template <typename CharT, typename Traits>
std::ostream&
operator<<(std::basic_ostream<CharT, Traits>& o, const dvo::DenseTracker::Stats& s)
{
    o << s.Levels.size () << " levels" << std::endl;

    for (dvo::DenseTracker::LevelStatsVector::const_iterator it = s.Levels.begin ();
         it != s.Levels.end ();
         ++it)
    {
        o << *it;
