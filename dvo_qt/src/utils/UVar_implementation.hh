#ifndef UVAR_IMPLEMENTATION_HH
#define UVAR_IMPLEMENTATION_HH

// TODO: Make the specialisation for matrices as well.
template <class T>
ValueHolder<T>*
UVar::register_new_gvar (const std::string& name, const T& default_val, int flags)
{
    ...
    registered_type_and_trait[name] =
    std::pair<BaseMap*, int> (&TypedMap<T>::instance (), flags);
    ...
    if (i == unmatched_tags.end ())
    {
        ...
        if (!(flags & SILENT))
            std::cerr << "? GV3::Register: " << type_name<T> () << " " << name
                      ...
                      << serialize::to_string (default_val, 0) << std::endl;
...
template <class T>
ValueHolder<T>* UVar::get_by_val (const std::string& name, const T& default_val, int flags)
{
...
template <class T>
ValueHolder<T>*
UVar::get_by_str (const std::string& name, const std::string& default_val, int flags)
{
    ValueHolder<T>* d = attempt_get<T> (name);
    if (d)
        return d;
...
template <>
inline std::string&
UVar::get<std::string> (const std::string& name, std::string default_val, int flags)
{
    return get_by_val (name, default_val, flags)->get ();
}

template <class T>
T& UVar::get (const std::string& name, std::string default_val, int flags)
{
    return get_by_str<T> (name, default_val, flags)->get ();
}

template <class T>
T& UVar::get (const std::string& name, const T& default_val, int flags)
{
    return get_by_val (name, default_val, flags)->get ();
}

#endif // UVAR_IMPLEMENTATION_HH
