...
{
    if (b) o << std::setprecision (20) << std::scientific;
}
...
template <typename T>
std::string to_string (const std::vector<T>& v, bool precise)
{
    std::ostringstream o;
    generic_setup (o, precise);
    o << "[ ";
...
template <int N> std::string to_string (const TooN::Vector<N>& m, bool precise)
{
    ...
    generic_setup (o, precise);
    o << "[ ";
    for (int i = 0; i < m.size (); i++) o << m[i] << " ";
    o << "]";
    return o.str ();
}

template <int N, int M>
std::string to_string (const TooN::Matrix<N, M>& m, bool precise)
{
    std::ostringstream o;
    generic_setup (o, precise);
    o << "[ ";
...
struct FromStream<TooN::Matrix<R, C, Precision>>
{
    static TooN::Matrix<R, C, Precision> from (std::istream& i)
    {
        ...
        if (i.fail () || i.bad ()) goto fail;

        for (unsigned int r = 1; r < v.size (); r++)
            if (v[r].size () != v[0].size ()) goto fail;

        if (R != -1 && v.size () != R) goto fail;

        if (C != -1 && v[0].size () != C) goto fail;
