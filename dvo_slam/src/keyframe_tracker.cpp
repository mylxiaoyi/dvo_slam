...
{
    ros::NodeHandle nh_;
    ros::Publisher ll_keyframe_pub_, ll_odometry_pub_, cn_odometry_pub_,
    cn_keyframe_pub_, de_odometry_pub_, de_keyframe_pub_, md_pub_;
...
public:
    ...
    : visualizer_ (visualizer), graph_ (), lt_ ()
    {
        if (visualizer_ != 0)
        {
            visualizer_->setGraph (&graph_);
            graph_.addMapChangedCallback (
            boost::bind (&KeyframeTracker::Impl::onGlobalMapChangedUpdateVisualization, this, _1));
        }

        // graph_.addMapChangedCallback(boost::bind(&KeyframeTracker::Impl::onGlobalMapChangedSaveTrajectory,
        // this, _1));

        lt_.addMapInitializedCallback (
        boost::bind (&KeyframeTracker::Impl::onMapInitialized, this, _1, _2, _3));

        lt_.addMapCompleteCallback (
        boost::bind (&KeyframeTracker::Impl::onMapComplete, this, _1, _2));

        lt_.addAcceptCallback (boost::bind (
        &KeyframeTracker::Impl::onAcceptCriterionTrackingResultEvaluation, this, _1, _2, _3));

        lt_.addAcceptCallback (boost::bind (
        &KeyframeTracker::Impl::onAcceptCriterionEstimateDivergence, this, _1, _2, _3));

        // lt_.addAcceptCallback(boost::bind(&KeyframeTracker::Impl::onAcceptCriterionEntropyRatio,
        // this, _1, _2, _3));

        lt_.addAcceptCallback (
        boost::bind (&KeyframeTracker::Impl::onAcceptCriterionDistance, this, _1, _2, _3));

        lt_.addAcceptCallback (
        boost::bind (&KeyframeTracker::Impl::onAcceptCriterionConstraintRatio, this, _1, _2, _3));

        lt_.addAcceptCallback (
        boost::bind (&KeyframeTracker::Impl::onAcceptCriterionConditionNumber, this, _1, _2, _3));
    ...
    void onMapInitialized (const LocalTracker& lt,
                           const LocalMap::Ptr& m,
                           const LocalTracker::TrackingResult& r_odometry)
    {
        // cv::imshow("keyframe", m->getKeyframe()->level(0).intensity /
        // 255.0f);
        // cv::waitKey(5);
    ...
    bool onAcceptCriterionTrackingResultEvaluation (const LocalTracker& lt,
                                                    const LocalTracker::TrackingResult& r_odometry,
                                                    const LocalTracker::TrackingResult& r_keyframe)
    {
        ...
        if (accept) evaluation->add (r_keyframe);
    ...
    bool onAcceptCriterionEstimateDivergence (const LocalTracker& lt,
                                              const LocalTracker::TrackingResult& r_odometry,
                                              const LocalTracker::TrackingResult& r_keyframe)
    {
        // std::cerr << "----->test "  << r_keyframe.Pose.translation().norm()
        // << " " << r_odometry.Pose.translation().norm() << std::endl;

        // dvo::core::AffineTransformd diff =
        // last_transform_to_keyframe_.inverse() * r_keyframe.Pose;

        std_msgs::Float64 md;
        // md.data = diff.translation().norm();
        // md_pub_.publish(md);

        bool reject1 = r_odometry.Transformation.translation ().norm () > 0.1 ||
                       r_keyframe.Transformation.translation ().norm () >
                       1.5 * cfg_.MaxTranslationalDistance;
        ...
        {
            std::cerr
            << "od before modify: " << r_odometry.Transformation.translation ().norm ()
            << std::endl;
            Eigen::Affine3d& p = const_cast<Eigen::Affine3d&>(r_odometry.Transformation);
            p.setIdentity ();
            std::cerr
            << "od after modify: " << r_odometry.Transformation.translation ().norm ()
            << std::endl;

            dvo::core::Matrix6d& information =
            const_cast<dvo::core::Matrix6d&>(r_odometry.Information);
            information.setIdentity ();
            information *= 0.008 * 0.008;

            std::cerr
            << "kf before modify: " << r_keyframe.Transformation.translation ().norm ()
            << std::endl;
            Eigen::Affine3d& p2 = const_cast<Eigen::Affine3d&>(r_keyframe.Transformation);
            p2 = last_transform_to_keyframe_;
            std::cerr
            << "kf after modify: " << r_keyframe.Transformation.translation ().norm ()
            << std::endl;
        }
    ...
    bool onAcceptCriterionDistance (const LocalTracker& lt,
                                    const LocalTracker::TrackingResult& r_odometry,
                                    const LocalTracker::TrackingResult& r_keyframe)
    {
    ...
    bool onAcceptCriterionConstraintRatio (const LocalTracker& lt,
                                           const LocalTracker::TrackingResult& r_odometry,
                                           const LocalTracker::TrackingResult& r_keyframe)
    {
        return (double(r_keyframe.Statistics.Levels.back ().Iterations.back ().ValidConstraints) /
                double(r_keyframe.Statistics.Levels.back ().ValidPixels)) >
               cfg_.MinEquationSystemConstraintRatio;
    }

    bool onAcceptCriterionConditionNumber (const LocalTracker& lt,
                                           const LocalTracker::TrackingResult& r_odometry,
                                           const LocalTracker::TrackingResult& r_keyframe)
    {
    ...
    void onGlobalMapChangedSaveTrajectory (KeyframeGraph& map)
    {
        ...
        dvo_slam::serialization::FileSerializer<dvo_slam::serialization::TrajectorySerializer> serializer (
        ss.str ());
    ...
    void update (const dvo::core::RgbdImagePyramid::Ptr& current,
                 const ros::Time& current_time,
                 Eigen::Affine3d& absolute_transformation)
    {
...
private:
    ...
    LocalTracker lt_;
    Eigen::Affine3d initial_transformation_, relative_transformation_,
    last_absolute_transformation_;
...
void KeyframeTracker::update (const dvo::core::RgbdImagePyramid::Ptr& current,
                              const ros::Time& current_time,
                              Eigen::Affine3d& absolute_transformation)
{
