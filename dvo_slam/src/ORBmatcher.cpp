...
// int ORBmatcher::SearchByProjection (std::shared_ptr<Frame> F,
...
//        // Apply ratio to second match (only if best and second are in the
//        same
//        // scale level)
//        if (bestDist <= TH_HIGH)
//        {
...
// int ORBmatcher::SearchByBoW (std::shared_ptr<KeyFrame> pKF,
//                             std::shared_ptr<Frame> F,
//                             vector<std::shared_ptr<MapPoint>>&
//                             vpMapPointMatches)
//{
//    const vector<std::shared_ptr<MapPoint>> vpMapPointsKF =
...
// int ORBmatcher::SearchByBoW2 (std::shared_ptr<KeyFrame> pKF1,
...
//    vpMapPointMatches = vector<std::shared_ptr<MapPoint>> (pKF2->N, nullptr);

//    const DBoW2::FeatureVector& vFeatVecKF = pKF1->mFeatVec;
...
//    // (at a certain level)
//    DBoW2::FeatureVector::const_iterator KFit = vFeatVecKF.begin ();
//    DBoW2::FeatureVector::const_iterator Fit = pKF2->mFeatVec.begin ();
//    DBoW2::FeatureVector::const_iterator KFend = vFeatVecKF.end ();
//    DBoW2::FeatureVector::const_iterator Fend = pKF2->mFeatVec.end ();
...
//                    const cv::Mat& dF = pKF2->mDescriptors.row (realIdxF);
...
//        else
//        {
//            Fit = pKF2->mFeatVec.lower_bound (KFit->first);
//        }
//    }
...
// int ORBmatcher::SearchByProjection (std::shared_ptr<KeyFrame> pKF,
//                                    cv::Mat Scw,
//                                    const
...
//    set<std::shared_ptr<MapPoint>> spAlreadyFound (vpMatched.begin (),
//    vpMatched.end ());
...
// int ORBmatcher::SearchForInitialization (std::shared_ptr<Frame> F1,
...
//                if (mbCheckOrientation)
//                {
//                    float rot = F1->mvKeysUn[i1].angle -
//                    F2->mvKeysUn[bestIdx2].angle;
//                    if (rot < 0.0) rot += 360.0f;
//                    int bin = round (rot * factor);
//                    if (bin == HISTO_LENGTH) bin = 0;
//                    assert (bin >= 0 && bin < HISTO_LENGTH);
...
// int ORBmatcher::SearchByBoW (std::shared_ptr<KeyFrame> pKF1,
//                             std::shared_ptr<KeyFrame> pKF2,
...
//    const vector<std::shared_ptr<MapPoint>> vpMapPoints2 =
//    pKF2->GetMapPointMatches ();
//    const cv::Mat& Descriptors2 = pKF2->mDescriptors;

//    vpMatches12 = vector<std::shared_ptr<MapPoint>> (
//    vpMapPoints1.size () /*,static_cast<MapPoint*>(NULL)*/);
...
int ORBmatcher::SearchForTriangulation(
  ...
  Eigen::Vector3d Cw = -pose1_wc.rotation() * pose1_cw.translation();
...
int ORBmatcher::Fuse(boost::shared_ptr<Keyframe> pKF,
  ...
  Eigen::Vector3d Ow = -pose_wc.rotation() * tcw;
  ...
  for (int i = 0; i < nMPs; i++) {
    ...
    Eigen::Vector3d p3Dw = pMP->GetPos();
    ...
    if (bestDist <= TH_LOW) {
      ...
      if (pMPinKF) {
        if (!pMPinKF->isBad()) {
          Map* pMap = Map::instance;

          if (pMPinKF->Observations() > pMP->Observations())
            //                        pMP->Replace (pMPinKF);
            pMap->ReplaceMapPoint(pMP, pMPinKF);
          else
            //                        pMPinKF->Replace (pMP);
            pMap->ReplaceMapPoint(pMPinKF, pMP);
        }
      } else {
...
// int ORBmatcher::Fuse (std::shared_ptr<KeyFrame> pKF,
//                      cv::Mat Scw,
...
//        const int nPredictedLevel = pMP->PredictScale (dist3D,
//        pKF->mfLogScaleFactor);
...
//        // If there is already a MapPoint replace otherwise add new
//        measurement
//        if (bestDist <= TH_LOW)
//        {
...
// int ORBmatcher::SearchBySim3 (std::shared_ptr<KeyFrame> pKF1,
//                              std::shared_ptr<KeyFrame> pKF2,
//                              vector<std::shared_ptr<MapPoint>>& vpMatches12,
//                              const float& s12,
//                              const cv::Mat& R12,
...
//        const int nPredictedLevel = pMP->PredictScale (dist3D,
//        pKF2->mfLogScaleFactor);

//        // Search in a radius
//        const float radius = th * pKF2->mvScaleFactors[nPredictedLevel];

//        const vector<size_t> vIndices = pKF2->GetFeaturesInArea (u, v,
//        radius);
...
//            if (kp.octave < nPredictedLevel - 1 || kp.octave >
//            nPredictedLevel)
//                continue;
...
//        // Compute predicted octave
//        const int nPredictedLevel = pMP->PredictScale (dist3D,
//        pKF1->mfLogScaleFactor);

//        // Search in a radius of 2.5*sigma(ScaleLevel)
//        const float radius = th * pKF1->mvScaleFactors[nPredictedLevel];

//        const vector<size_t> vIndices = pKF1->GetFeaturesInArea (u, v,
//        radius);
...
int ORBmatcher::SearchByProjection(KeyframePtr& CurrentFrame,
  ...
  Eigen::Affine3d current_pose_wc = CurrentFrame->pose();
  Eigen::Affine3d current_pose_cw = current_pose_wc.inverse();
  //    Eigen::Affine3d last_pose_wc = LastFrame->pose();
  ...
  for (int i = 0; i < LastFrame->N; i++) {
    ...
    if (pMP) {
      if (!LastFrame->mvbOutlier[i]) {
        ...
        Eigen::Vector3d x3Dw = pMP->GetPos();
        Eigen::Vector3d x3Dc = current_pose_cw * x3Dw;
        ...
        Eigen::Vector2d uv = CurrentFrame->project(x3Dc);

        //                if (u < CurrentFrame->mnMinX || u >
        //                CurrentFrame->mnMaxX)
        //                    continue;
        //                if (v < CurrentFrame->mnMinY || v >
        //                CurrentFrame->mnMaxY)
        //                    continue;
        if (!CurrentFrame->isInImageBound(uv)) {
          continue;
        }
        ...
        //                if (bForward)
        //                    vIndices2 = CurrentFrame->GetFeaturesInArea (u, v,
        //                    radius, nLastOctave);
        //                else if (bBackward)
        //                    vIndices2 = CurrentFrame->GetFeaturesInArea (u, v,
        //                    radius, 0, nLastOctave);
        //                else
        ...
        for (vector<size_t>::const_iterator vit = vIndices2.begin(),
          ...
          //                    {
          //                        const float ur = u - CurrentFrame->mbf *
          //                        invzc;
          //                        const float er = fabs (ur -
        ...
        if (bestDist <= TH_HIGH) {
          ...
          if (mbCheckOrientation) {
            ...
            float rot = LastFrame->mvKeys[i].angle -
...
// int ORBmatcher::SearchByProjection (std::shared_ptr<Frame> CurrentFrame,
...
//                const float u = CurrentFrame->fx * xc * invzc +
//                CurrentFrame->cx;
//                const float v = CurrentFrame->fy * yc * invzc +
//                CurrentFrame->cy;
...
//                CurrentFrame->GetFeaturesInArea (u, v, radius, nPredictedLevel
//                - 1,
//                                                 nPredictedLevel + 1);
