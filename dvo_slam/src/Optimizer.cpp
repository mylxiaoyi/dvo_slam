...
void Optimizer::GlobalBundleAdjustemnt(boost::shared_ptr<Map> pMap,
                                       int nIterations, bool* pbStopFlag,
                                       const unsigned long nLoopKF,
                                       const bool bRobust) {
  vector<boost::shared_ptr<Keyframe>> vpKFs = pMap->GetAllKeyFrames();
  vector<boost::shared_ptr<MapPoint>> vpMP = pMap->GetAllMapPoints();
  BundleAdjustment(vpKFs, vpMP, nIterations, pbStopFlag, nLoopKF, bRobust);
}

void Optimizer::BundleAdjustment(
    const vector<boost::shared_ptr<Keyframe>>& vpKFs,
    const vector<boost::shared_ptr<MapPoint>>& vpMP, int nIterations,
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    boost::shared_ptr<Keyframe> pKF = vpKFs[i];
    if (pKF->isBad()) continue;
    g2o::VertexSE3Expmap* vSE3 = new g2o::VertexSE3Expmap();
    //    vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose()));
    vSE3->setEstimate(toSE3Quat(pKF->pose().inverse()));
    vSE3->setId(pKF->id());
    vSE3->setFixed(pKF->id() == 1);
  ...
  for (size_t i = 0; i < vpMP.size(); i++) {
    boost::shared_ptr<MapPoint> pMP = vpMP[i];
    ...
    int nEdges = 0;
    // SET EDGES
    for (map<boost::shared_ptr<Keyframe>, size_t>::const_iterator mit =
             observations.begin();
...
void Optimizer::LocalBundleAdjustment(boost::shared_ptr<Keyframe> pKF,
  ...
  for (list<boost::shared_ptr<MapPoint>>::iterator
    ...
    map<boost::shared_ptr<Keyframe>, size_t> observations =
        (*lit)->GetObservations();
    for (map<boost::shared_ptr<Keyframe>, size_t>::iterator
...
void Optimizer::OptimizeEssentialGraph(
    ...
    const map<boost::shared_ptr<Keyframe>, set<boost::shared_ptr<Keyframe>>>&
        LoopConnections,
  ...
  const vector<boost::shared_ptr<Keyframe>> vpKFs = pMap->GetAllKeyFrames();
  const vector<boost::shared_ptr<MapPoint>> vpMPs = pMap->GetAllMapPoints();
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    if (it != CorrectedSim3.end()) {
      ...
      Eigen::Matrix<double, 3, 1> tcw = pKF->pose().inverse().translation();
  ...
  // Set Loop edges
  for (map<boost::shared_ptr<Keyframe>,
           set<boost::shared_ptr<Keyframe>>>::const_iterator
           mit = LoopConnections.begin(),
           mend = LoopConnections.end();
       mit != mend; mit++) {
    boost::shared_ptr<Keyframe> pKF = mit->first;
    const long unsigned int nIDi = pKF->id();
    const set<boost::shared_ptr<Keyframe>>& spConnections = mit->second;
    const g2o::Sim3 Siw = vScw[nIDi];
    const g2o::Sim3 Swi = Siw.inverse();

    for (set<boost::shared_ptr<Keyframe>>::const_iterator
             sit = spConnections.begin(),
  ...
  for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
    ...
    boost::shared_ptr<Keyframe> pParentKF = pKF->GetParent();
    ...
    // Loop edges
    const set<boost::shared_ptr<Keyframe>> sLoopEdges = pKF->GetLoopEdges();
    for (set<boost::shared_ptr<Keyframe>>::const_iterator
             sit = sLoopEdges.begin(),
             send = sLoopEdges.end();
         sit != send; sit++) {
      boost::shared_ptr<Keyframe> pLKF = *sit;
      if (pLKF->id() < pKF->id()) {
        g2o::Sim3 Slw;
    ...
    const vector<boost::shared_ptr<Keyframe>> vpConnectedKFs =
        pKF->GetCovisiblesByWeight(minFeat);
    for (vector<boost::shared_ptr<Keyframe>>::const_iterator vit =
             vpConnectedKFs.begin();
         vit != vpConnectedKFs.end(); vit++) {
      boost::shared_ptr<Keyframe> pKFn = *vit;
      if (pKFn && pKFn != pParentKF && !pKF->hasChild(pKFn) &&
          !sLoopEdges.count(pKFn)) {
        if (!pKFn->isBad() && pKFn->id() < pKF->id()) {
          if (sInsertedEdges.count(make_pair(min(pKF->id(), pKFn->id()),
                                             max(pKF->id(), pKFn->id()))))
            continue;
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    boost::shared_ptr<Keyframe> pKFi = vpKFs[i];
    ...
    Eigen::Affine3d Tiw(eigR);
    Tiw.translation() = eigt;

    //    pKFi->SetPose(Tiw);
    pKFi->pose(Tiw.inverse());
  }
...
int Optimizer::OptimizeSim3(
    ...
    std::vector<boost::shared_ptr<MapPoint>>& vpMatches1, g2o::Sim3& g2oS12,
  ...
  const Eigen::Matrix<double, 3, 3>& K1 = pKF1->mK;
  const Eigen::Matrix<double, 3, 3>& K2 = pKF2->mK;
  ...
  const Eigen::Matrix<double, 3, 3> R1w = pKF1->pose().inverse().rotation();
  const Eigen::Vector3d t1w = pKF1->pose().inverse().translation();
  const Eigen::Matrix<double, 3, 3> R2w = pKF2->pose().inverse().rotation();
  const Eigen::Vector3d t2w = pKF2->pose().inverse().translation();
  ...
  for (int i = 0; i < N; i++) {
    ...
    boost::shared_ptr<MapPoint> pMP1 = vpMapPoints1[i];
    boost::shared_ptr<MapPoint> pMP2 = vpMatches1[i];
...
// int Optimizer::OptimizeSim3FitGPS (std::vector<boost::shared_ptr<KeyFrame>>
// &vpKFs,
