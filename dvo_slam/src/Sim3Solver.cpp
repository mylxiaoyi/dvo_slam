...
Sim3Solver::Sim3Solver(boost::shared_ptr<Keyframe> pKF1,
                       boost::shared_ptr<Keyframe> pKF2,
                       const vector<boost::shared_ptr<MapPoint>> &vpMatched12,
                       const bool bFixScale)
    : mnIterations(0), mnBestInliers(0), mbFixScale(bFixScale) {
  mpKF1 = pKF1;
  mpKF2 = pKF2;

  vector<boost::shared_ptr<MapPoint>> vpKeyFrameMP1 =
  ...
  Eigen::Affine3d pose1_cw = pKF1->pose().inverse();
  Eigen::Affine3d pose2_cw = pKF2->pose().inverse();
  ...
  for (int i1 = 0; i1 < mN1; i1++) {
    if (vpMatched12[i1]) {
      boost::shared_ptr<MapPoint> pMP1 = vpKeyFrameMP1[i1];
      boost::shared_ptr<MapPoint> pMP2 = vpMatched12[i1];
      ...
      mvX3Dc1.push_back(pose1_cw * X3D1w);

      Eigen::Vector3d X3D2w = pMP2->GetWorldPos();
      //            mvX3Dc2.push_back(Rcw2*X3D2w+tcw2);
      mvX3Dc2.push_back(pose2_cw * X3D2w);
...
cv::Mat Sim3Solver::iterate(int nIterations, bool &bNoMore,
  ...
  while (mnIterations < mRansacMaxIts && nCurrentIterations < nIterations) {
    ...
    for (short i = 0; i < 3; ++i) {
      ...
      P3Dc1i.col(i).at<float>(0) = mvX3Dc1[idx](0);
      P3Dc1i.col(i).at<float>(1) = mvX3Dc1[idx](1);
      P3Dc1i.col(i).at<float>(2) = mvX3Dc1[idx](2);

      P3Dc2i.col(i).at<float>(0) = mvX3Dc2[idx](0);
      P3Dc2i.col(i).at<float>(1) = mvX3Dc2[idx](1);
      P3Dc2i.col(i).at<float>(2) = mvX3Dc2[idx](2);
...
void Sim3Solver::CheckInliers() {
  ...
  mnInliersi = 0;
    ...
    for (size_t i = 0; i < mvP1im1.size(); i++) {
      ...
      const float err1 = dist1.transpose() * dist1;
      const float err2 = dist2.transpose() * dist2;
...
void Sim3Solver::Project(const vector<Eigen::Vector3d> &vP3Dw, vector<Eigen::Vector2d> &vP2D,
                         cv::Mat Tcw, Eigen::Matrix<double, 3, 3> K) {
//  cv::Mat Rcw = Tcw.rowRange(0, 3).colRange(0, 3);
//  cv::Mat tcw = Tcw.rowRange(0, 3).col(3);
  Eigen::Matrix<double, 3, 3> Rcw;
  Rcw << Tcw.at<float>(0, 0), Tcw.at<float>(0, 1), Tcw.at<float>(0, 2),
         Tcw.at<float>(1, 0), Tcw.at<float>(1, 1), Tcw.at<float>(1, 2),
         Tcw.at<float>(2, 0), Tcw.at<float>(2, 1), Tcw.at<float>(2, 2);
  Eigen::Vector3d tcw(Tcw.at<float>(0, 3), Tcw.at<float>(1, 3), Tcw.at<float>(2, 3));
...
void Sim3Solver::FromCameraToImage(const std::vector<Eigen::Vector3d> &vP3Dc,
