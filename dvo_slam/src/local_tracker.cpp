...
LocalTracker::LocalTracker () : impl_ (new internal::LocalTrackerImpl ())
{
...
LocalTracker::addMapCompleteCallback (const MapCompleteCallback& callback)
{
    return impl_->map_complete_.connect (callback);
}

boost::signals2::connection
LocalTracker::addMapInitializedCallback (const MapInitializedCallback& callback)
{
...
void LocalTracker::configure (const dvo::DenseTracker::Config& config)
{
    ...
    if (impl_->predicate.intensity_threshold != config.IntensityDerivativeThreshold ||
...
void LocalTracker::initNewLocalMap (const dvo::core::RgbdImagePyramid::Ptr& keyframe,
...
void LocalTracker::update (const dvo::core::RgbdImagePyramid::Ptr& image,
                           dvo::core::AffineTransformd& pose)
{
    ...
    // TODO: fix me!
    boost::function<void()> h1 = boost::bind (&internal::LocalTrackerImpl::match,
                                              impl_->keyframe_tracker_,
                                              impl_->keyframe_points_,
                                              image,
                                              &r_keyframe);
    boost::function<void()> h2 = boost::bind (&internal::LocalTrackerImpl::match,
                                              impl_->odometry_tracker_,
                                              impl_->active_frame_points_,
                                              image,
                                              &r_odometry);
    ...
    else
    {
        ...
        // TODO: if we have a tracking failure in odometry and in keyframe this
        // initialization makes no sense
