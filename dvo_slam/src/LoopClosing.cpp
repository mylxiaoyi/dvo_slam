...
// void LoopClosing::SetTracker(boost::shared_ptr<Tracking> pTracker) {
//  mpTracker = pTracker;
//}

// void LoopClosing::SetLocalMapper(boost::shared_ptr<LocalMapping>
// pLocalMapper) {
//  mpLocalMapper = pLocalMapper;
//}

void LoopClosing::execLoop() { run(); }
...
void LoopClosing::InsertKeyFrame(boost::shared_ptr<Keyframe> pKF) {
  unique_lock<mutex> lock(mMutexLoopQueue);
  if (pKF->id() != 1) mlpLoopKeyFrameQueue.push_back(pKF);
}
...
bool LoopClosing::DetectLoop() {
  ...
  const vector<boost::shared_ptr<Keyframe>> vpConnectedKeyFrames =
      mpCurrentKF->GetVectorCovisibleKeyFrames();
  const DBoW2::BowVector& CurrentBowVec = mpCurrentKF->mBowVec;
  float minScore = 1;
  for (size_t i = 0; i < vpConnectedKeyFrames.size(); i++) {
    boost::shared_ptr<Keyframe> pKF = vpConnectedKeyFrames[i];
  ...
  for (size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++) {
    boost::shared_ptr<Keyframe> pCandidateKF = vpCandidateKFs[i];

    set<boost::shared_ptr<Keyframe>> spCandidateGroup =
    ...
    for (size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++) {
      set<boost::shared_ptr<Keyframe>> sPreviousGroup =
          mvConsistentGroups[iG].first;
...
bool LoopClosing::ComputeSim3() {
  ...
  for (int i = 0; i < nInitialCandidates; i++) {
    boost::shared_ptr<Keyframe> pKF = mvpEnoughConsistentCandidates[i];
    ...
    for (int i = 0; i < nInitialCandidates; i++) {
      ...
      if (!Scm.empty()) {
        vector<boost::shared_ptr<MapPoint>> vpMapPointMatches(
            vvpMapPointMatches[i].size(), nullptr);
        ...
        if (nInliers >= 20) {
          bMatch = true;
          mpMatchedKF = pKF;
          g2o::Sim3 gSmw(pKF->pose().inverse().rotation(),
                         pKF->pose().inverse().translation(), 1.0);
          mg2oScw = gScm * gSmw;
...
void LoopClosing::CorrectLoop() {
  ...
  Eigen::Affine3d Twc = mpCurrentKF->pose();

  {
    ...
    for (vector<boost::shared_ptr<Keyframe>>::iterator
      ...
      Eigen::Affine3d Tiw = pKFi->pose().inverse();

      if (pKFi != mpCurrentKF) {
        Eigen::Affine3d Tic = Tiw * Twc;
        //        cv::Mat Ric = Tic.rowRange(0, 3).colRange(0, 3);
        //        cv::Mat tic = Tic.rowRange(0, 3).col(3);
        Eigen::Matrix<double, 3, 3> Ric = Tic.rotation();
        Eigen::Vector3d tic = Tic.translation();
        g2o::Sim3 g2oSic(Ric, tic, 1.0);
      ...
      Eigen::Matrix<double, 3, 3> Riw = Tiw.rotation();
      Eigen::Vector3d tiw = Tiw.translation();
      g2o::Sim3 g2oSiw(Riw, tiw, 1.0);
    ...
    for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
      ...
      vector<boost::shared_ptr<MapPoint>> vpMPsi = pKFi->GetMapPointMatches();
      for (size_t iMP = 0, endMPi = vpMPsi.size(); iMP < endMPi; iMP++) {
        boost::shared_ptr<MapPoint> pMPi = vpMPsi[iMP];
        if (!pMPi) continue;
        ...
        Eigen::Vector3d cvCorrectedP3Dw = eigCorrectedP3Dw;
      ...
      Eigen::Affine3d correctedTiw(eigR);
      correctedTiw.translation() = eigt;

      //      pKFi->SetPose(correctedTiw);
      pKFi->pose(correctedTiw.inverse());
    ...
    for (size_t i = 0; i < mvpCurrentMatchedPoints.size(); i++) {
      if (mvpCurrentMatchedPoints[i]) {
        boost::shared_ptr<MapPoint> pLoopMP = mvpCurrentMatchedPoints[i];
        boost::shared_ptr<MapPoint> pCurMP = mpCurrentKF->GetMapPoint(i);
        if (pCurMP)
  ...
  map<boost::shared_ptr<Keyframe>, set<boost::shared_ptr<Keyframe>>>
      LoopConnections;
  ...
  for (vector<boost::shared_ptr<Keyframe>>::iterator
       ...
       vit != vend; vit++) {
    boost::shared_ptr<Keyframe> pKFi = *vit;
    vector<boost::shared_ptr<Keyframe>> vpPreviousNeighbors =
...
void LoopClosing::SearchAndFuse(const KeyFrameAndPose& CorrectedPosesMap) {
  ...
  for (KeyFrameAndPose::const_iterator mit = CorrectedPosesMap.begin(),
    ...
    vector<boost::shared_ptr<MapPoint>> vpReplacePoints(mvpLoopMapPoints.size(),
                                                        nullptr);
    ...
    for (int i = 0; i < nLP; i++) {
      boost::shared_ptr<MapPoint> pRep = vpReplacePoints[i];
      if (pRep) {
...
//      }
      ...
      list<boost::shared_ptr<Keyframe>> lpKFtoCheck(
          mpMap->mvpKeyFrameOrigins.begin(), mpMap->mvpKeyFrameOrigins.end());

      while (!lpKFtoCheck.empty()) {
        boost::shared_ptr<Keyframe> pKF = lpKFtoCheck.front();
        const set<boost::shared_ptr<Keyframe>> sChilds = pKF->GetChilds();
//        cv::Mat Twc = pKF->GetPoseInverse();
        Eigen::Affine3d Twc = pKF->pose();
        for (set<boost::shared_ptr<Keyframe>>::const_iterator sit =
                 sChilds.begin();
             sit != sChilds.end(); sit++) {
          boost::shared_ptr<Keyframe> pChild = *sit;
          if (pChild->mnBAGlobalForKF != nLoopKF) {
//            cv::Mat Tchildc = pChild->GetPose() * Twc;
            Eigen::Affine3d Tchildc = pChild->pose().inverse() * Twc;
...
//        pKF->mTcwBefGBA = pKF->GetPose();
        pKF->mTcwBefGBA = pKF->pose().inverse();
//        pKF->SetPose(pKF->mTcwGBA);
        pKF->pose(pKF->mTcwGBA.inverse());
        lpKFtoCheck.pop_front();
      }

      // Correct MapPoints
      const vector<boost::shared_ptr<MapPoint>> vpMPs =
          mpMap->GetAllMapPoints();

      for (size_t i = 0; i < vpMPs.size(); i++) {
        boost::shared_ptr<MapPoint> pMP = vpMPs[i];
        ...
        if (pMP->mnBAGlobalForKF == nLoopKF) {
          ...
          // keyframe
          boost::shared_ptr<Keyframe> pRefKF = pMP->GetReferenceKeyFrame();
...
//          cv::Mat Xc = Rcw * pMP->GetWorldPos() + tcw;
          Eigen::Matrix<double, 3, 3> Rcw = pRefKF->mTcwBefGBA.rotation();
          Eigen::Vector3d tcw = pRefKF->mTcwBefGBA.translation();
          Eigen::Vector3d Xc = Rcw * pMP->GetWorldPos() + tcw;

          // Backproject using corrected camera
//          cv::Mat Twc = pRefKF->GetPoseInverse();
//          cv::Mat Rwc = Twc.rowRange(0, 3).colRange(0, 3);
//          cv::Mat twc = Twc.rowRange(0, 3).col(3);
          Eigen::Affine3d Twc = pRefKF->pose();
          Eigen::Matrix<double, 3, 3> Rwc = Twc.rotation();
